...
class ContentGenerator:
    ...
    def generate_blog_post(self, topic, prompt_template):
        ...
        Args:
            ...
            prompt_template (str): The full prompt string to send to the AI.
            
        Returns:
            dict: JSON response with title, content, tags, etc.
        """
        
        # Replace the {topic} placeholder in the template with the actual topic
        # We assume the template uses {topic} for formatting.
        # However, since the user might edit it freely, we should double check if formatting is safe.
        # But for simplicity, we'll try to safe format or just pass it to the model instruction if structured differently.
        # Based on current templates, they use {topic}. 
