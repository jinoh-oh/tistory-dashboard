...
class ContentGenerator:
    def __init__(self):
        ...
        # Use a system instruction to enforce length and quality constraints
        self.model = genai.GenerativeModel(
            ...
            다음 지침을 철저히 준수하세요:
            1. 분량: 본문 텍스트 내의 **한글 글자 수(공백 제외)가 반드시 1,600자~2,000자 사이**가 되도록 매우 길고 상세하게 작성하세요.
    ...
    def generate_blog_post(self, topic, prompt_template):
        ...
        # Add a final reminder about length to the user prompt as well
        prompt += "\n\n⚠️ 중요: 반드시 한글 1,600자 이상의 충분한 분량으로 작성하세요. 내용이 짧으면 탈락입니다."

        try:
            response = self.model.generate_content(prompt, generation_config={"response_mime_type": "application/json"})
            data = json.loads(response.text)
            
            # Simple check for content length to warn the console/logs
            content = data.get('content', '')
            clean_text = re.sub(r'<[^>]+>', '', content)
            char_count = len(re.sub(r'\s+', '', clean_text))
            
            if char_count < 1500:
                print(f"Warning: Content length is only {char_count} chars. Retrying internal expansion...")
                # Note: For production, we might want to do a real retry here, but starting with prompt optimization.
